...
export class SalesComponent implements OnInit {
  ...
  exportarPDFUnaSolaVenta(idCliente: number, idVenta: number) {
    try {
      this.saleService.getPropertiesClient(idCliente, idVenta).subscribe(async (response: any) => {
        if ((await response.Status) == 200) {
          ...
          setTimeout(() => {
            alert('Error al intentar exportar la venta');
            location.reload();
          }, 1000);
          throw new Error('Error al intentar exportar la venta');
        }
      });
    } catch (error) {
      console.log(error);
      throw new Error('Error al intentar ejecutar la query');
    }
